...
Incremental GC — use `game.begin()` / `game.end()` (or `gcframestep(1.0)` twice per frame in raw loops). Avoid per-frame color object literals, uncached HUD strings, and short field names (`x`, `y`) that clash with locals like `ballx` (identifiers are case-insensitive). Use `colors.*` for draws and `let vx: float` for hot physics math.
