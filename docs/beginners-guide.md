...
| `map` / object | `{ x: 1 }` | JSON/config tables |
| `func` | `func(x) { return x; }` | Callable value |
| `null` | `null` | “No value” |
...
**String interpolation** — embed values without manual concatenation:

```koda
let score = 0;
let lives = 3;
drawtext("Score: {score}   Lives: {lives}", 20, 20, 24, colors.white);
```
...
For game states, **`match`** is cleaner — each arm is a block with no fall-through:
...
func main() {
    ...
    while (game.running()) {
        ...
        game.text("Hello, Koda!", 300, 280, 30, colors.white);
        game.end();
    }
}
```
