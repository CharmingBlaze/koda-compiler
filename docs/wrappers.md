...
**Full guide:** [Wrapping any C library](guides/wrapping-libraries.md)

Koda stays on a **single native pipeline**: the Go frontend lowers programs to **LLVM IR**, then **`koda build`** runs **llc** on that IR and links the resulting object with **`runtime/libkoda_runtime.a`** (C sources under `runtime/src/`) using **Clang** as the driver, plus optional **`KODA_NATIVE_SOURCES`** / **`KODA_LINKFLAGS`** for your C glue. Wrappers are **ordinary `.koda` modules** (plus your own glue) that you ship alongside—or inside—a **wrapper root directory** so `import "@raylib"` resolves without vendoring copies into every app.

## Generated package (kodawrap)

Each `koda wrap` run produces an **organized library folder**:

| Artifact | Purpose |
|----------|---------|
| `<name>.koda` | Categorized bindings |
| `wrapper.c` | Native glue |
| `README.md` | Quick start + link flags |
| `api_reference.md` | Markdown API |
| `examples.md` | Sample calls |
| `koda.json` | Merge `native` into your project |
| `META.json` | Tooling metadata |
| `docs/index.html` | Searchable offline docs |
...
- **`kodawrap`** — build as `go build -o kodawrap ./cmd/wrapgen`. Uses **clang** when available for accurate parsing, with regex fallback. Emits organized packages with HTML docs.
- **`koda wrap …`** — forwards to **`kodawrap`** next to **`koda`** or on **`PATH`**.
- See **[guides/wrapping-libraries.md](guides/wrapping-libraries.md)** for wrapping **any** C library.

```bash
koda wrap -name mylib -headers ./include/mylib.h \
  -I ./include -L ./lib -l mylib -o wrappers/mylib
```
