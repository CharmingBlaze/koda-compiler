...
$env:KODA_BUNDLE_FILES = '.\raylib.dll;.\LICENSE.txt;.\assets'
.\koda.exe bundle .\game.koda -o .\dist\game
```

```bash
# Linux/macOS path-list style (:)
KODA_BUNDLE_FILES="./libfoo.so:./assets:./LICENSE.txt" koda bundle game.koda -o dist/game
```
...
| Extra files | Any files or directories listed in `KODA_BUNDLE_FILES`. |
...
After you have release **`koda.exe`** / **`kodawrap.exe`** (embedded Clang + llc + lld + runtime), assemble the full distributable folder:
